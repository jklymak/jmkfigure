...
def bindata(binx,biny,x,y,X):
    ...
    indx = np.floor(np.interp(x,binx, np.arange(0,N+1,1.0))).astype(int)
    indx[indx<0] = 0
    indx[indx>N-1] = N - 1


    indy = np.floor(np.interp(y, biny, np.arange(0,M+1,1.0))).astype(int)
    #print shape(X)
    indy[indy<0]=0
    indy[indy > M-1] = M - 1
    ...
    varX=varX/nX
    # mask
    meanX=np.ma.masked_invalid(meanX)
    nX=np.ma.masked_invalid(nX)
    varX=np.ma.masked_invalid(varX)
...
def bindata1d(binx, x, X):
    ...
    y = x * 0. + 1.
    meanX, varX, nX = bindata(binx, biny, x, y, X)
    return np.squeeze(meanX), np.squeeze(varX), np.squeeze(nX)
